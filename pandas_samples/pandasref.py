...
# pd.read_table('https://raw.githubusercontent.com/justmarkham/DAT7/master/data/drinks.csv')
# drink_cols = ['country', 'beer_servings', 'spirit_servings', 'wine_servings', 'total_litres_of_pure_alcohol',' continent']
...
drinks.dtypes            # data types of each columndr
drinks.shape             # number of rows and columns
drinks.values            # underlying numpy array
drinks.info()            # concise summary (including memory usage)
...
users.groupby('occupation').agg(['count'])
users.groupby('occupation').count() # same as above
